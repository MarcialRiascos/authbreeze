...
class CitadController extends Controller
{
    public function __construct()
    {
        $this->middleware('can:citad.index')->only('index');
        $this->middleware('can:citad.create')->only('create');
        $this->middleware('can:citad.store')->only('store');
        $this->middleware('can:citad.destroy')->only('destroy');
    }
    ...
    public function index()
    {
        ...
        $citassspe = Event::all()->where('start_date', '>',  $fechaa);
        ...
        return view('cita', compact('users', 'events', 'citasss', 'citasssp', 'citasssd', 'citassspe', 'citas', 'usuarios', 'services'));
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($fecha1->lt($fecha2)) {
            ...
            return back();
           // return redirect()->route('citad.store',  compact('users', 'events', 'citasss', 'citasssp', 'citasssd', 'citassspe', 'citas', 'usuarios', 'services'))->with('danger', 'La fecha y hora que esta introduciendo es anterior a la actulidad, por favor ingrese una fecha valida');
        } else {
            if (!$citass->isEmpty()) {
                ...
                foreach ($citass as $cita) {
                    ...
                    return back();
                   // return redirect()->route('citad.store',  compact('users', 'events', 'citasss', 'citasssp', 'citasssd', 'citassspe', 'citas', 'usuarios', 'services'))->with('danger', 'No hay agenda para la fecha, hora y empleado solicitado, intente consultar con otro empleado para agendar su cita');
                }
            } else {
                ...
                return back();
                //return redirect()->route('citad.store',  compact('users', 'events', 'citasss', 'citasssp', 'citasssd', 'citassspe', 'citas', 'usuarios', 'services'))->with('success', 'Si hay agenda para la fecha, hora y empleado solicitado, puede agendar su cita');
            }
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($citasi >= 2) {
           ...
           return back();
          // return redirect()->route('citad.store',  compact('users', 'events', 'citasss', 'citasssp', 'citasssd', 'citassspe', 'citas', 'usuarios', 'services'))->with('danger', 'Cita no agedada, puede que ya tenga 2 citas pendientes, el empleado solicitado se encuentra ocupado para la fecha y hora solicitada, o la fecha indroducida no es valida');
        } else {
            if ($fecha1->gt($fecha2)) {
                ...
                if ($cita == 0) {
                    ...
                    if ($cital == 0) {
                        ...
                        return back();
                       // return redirect()->route('citad.store',  compact('users', 'events', 'citasss', 'citasssp', 'citasssd', 'citassspe', 'citas', 'usuarios', 'services'))->with('success', 'Cita Agendada con Exito');
                    } else {
    ...
    public function destroy(Request $request)
    {
        ...
        if ($dato) {
            ...
            $dato->delete();
                ...
                return back();
               // return redirect()->route('citad.store',  compact('users', 'events', 'citasss', 'citasssp', 'citasssd', 'citassspe', 'citas', 'usuarios', 'services'))->with('success', 'Cita cancelada con exito');
            
        } else {
                   // return view('citas', compact('users', 'events', 'citasss', 'citasssp', 'citasssd', 'citassspe', 'citas'));
                   Alert::error('¡Error!', 'Su cita no fue cancelada')->autoClose(5000); 
                   return back();
                  // return redirect()->route('citad.store',  compact('users', 'events', 'citasss', 'citasssp', 'citasssd', 'citassspe', 'citas', 'usuarios', 'services'))->with('danger', 'Error al cancelar su cita');
        }
